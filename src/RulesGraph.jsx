...
const getNodes = (targets) => {
    ...
    for (const role of targets.delegations.roles) {
        nodes.push({ id: role.name, label: role.name, color: "#ff6666"})
        if (role.keyids === null) continue;
        for (const keyid of role.keyids) {
            if (!nodes.some((node) => node.id === keyid)) {
                console.log(keyid, keyid.length);
                
                const nodeData = {
                    id: keyid, 
                    label: keyid.length > 40 ? keyid.substring(0, 40) + "..." : keyid
                }
...
const RulesGraph = (props) => {
    ...
    const options = {
        ...
        layout: {
            hierarchical: {
                ...
                treeSpacing: 250, // space between trees
    ...
    useEffect(() => {
        if (networkRef.current && !networkInstanceRef.current) {
            networkInstanceRef.current = new Network(networkRef.current, { nodes, edges, options }, options);
